...
- **8ファクターとの相互参照（`pca.correlate_with_factors`）**: 層1で自前計算
  した8ファクター（Value/Growth/Momentum/Quality/Size/Beta/Volatility/
  Liquidity）のリターンと、層3のPCAスコア（直近で最も動いた主成分）との
  直近60日相関を計算し、PCA解説文に組み込んだ。実データで検証したところ、
  トレンドが強い局面ではPC1が8ファクターの多く（Momentum・Beta・Liquidity等）
  と同時に強く相関することが分かった（ファクター同士も互いに相関しやすいため）。
  そのため相関上位2件のみを表示し、3件以上が閾値|0.4|を超える場合は
  「単一ファクターへの還元には注意」と明記して読み手を誤誘導しないようにした。
- **market_regime.py（新規実装・日次レポートへ統合済み）**: PCAを補完する、
  ルールベースで直感的な指標として新規に実装した。3つの軸で判定する。
  1. トレンド: 終値と50日/200日移動平均の位置関係（上昇/下降/もみ合い）
  2. ボラティリティ: 直近20日の実現ボラティリティ(年率)の、過去1年分布内
     でのパーセンタイル（低/平常/高ボラ）
  3. セクター間相関: 直近20日のセクターETF間平均相関（高相関=一斉に動く
     相場、低相関=個別色の強い相場）
  週次で軸を保存するPCAとは異なり、実行のたびに直近データから計算し直す
  （計算コストが軽いため状態ファイルは持たない）。日次レポートの見出し
  （headline）の直下に「相場のレジーム（地合い）」ボックスとして表示。
- **結論・主因の見立てボックス（`report.build_lead_narrative`）**: ユーザーが
  「何主導で動いたのかが分かりにくい、項目が多すぎる」とフィードバック。
  他のAIツールに同じ日の分析を依頼した際の「結論→主因（事実+出典）→セクター→
  個別銘柄→解釈（確度付き）」という構成が読みやすいとの指摘を受け、既存の
  自前データを同じ構成に組み直してページ冒頭に表示するようにした。
  - 主因: 指数リターン・最大変動ファクター・その日のマクロ変数の実際の動き
    （`macro.today_macro_moves`。USDJPY/米金利/原油、一定以上動いた場合のみ）・
    PCAの支配的パターンのセクター解釈、をすべて事実（自前の定量データ）として列挙。
  - セクター/個別銘柄: 寄与度・リターン上位下位2件ずつを一行で提示。
  - 解釈: PCAの残差比率・マクロ相関・ファクター相関・ニュース裏付け件数のうち
    いくつが確認できたかで確度（高/中/低）を機械的に判定し明記する
    （「それっぽい断定」を避け、根拠の強さを常に併記する設計）。
  - 詳細セクション（本日の論点・全セクター・ファクター表・PCA詳細等）は
    `<details>`（JS不要のネイティブ折りたたみ）で初期状態は閉じておき、
    冒頭は結論・主因・解釈だけが目に入るようにして情報量を絞った。
  - 制約: 主因・セクター・個別銘柄は完全に定量ベースなので毎日必ず表示できるが、
    ニュース引用（定性面）は無料RSSキーワード一致で見つかった場合のみ。他AIの
    分析にあった「サウジのパイプライン停止」のような具体的な実時間ニュースの
    引用は、`ANTHROPIC_API_KEY`を設定してAPIベースの`news_lookup_batch.py`
    （Web検索）を有効化しないと再現できない（無料RSSの精度限界は既述のとおり）。
...
daily-review/
  src/
    ...
    market_regime.py               レジーム判定（トレンド・ボラティリティ・セクター間相関の3軸、PCAを補完）
