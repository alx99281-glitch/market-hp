...
- **市場の厚み（breadth）**: `decompose.breadth_metrics()` で値上がり/値下がり
  銘柄数を計算し、z score異常検知・日次headline両方に統合（例:「値上がり59/
  値下がり165銘柄」）。PC1説明力と近い情報をより直感的に示す指標として、
  ユーザーの提案で追加した。
- **マクロ変数相関（`macro.py`）**: USDJPY・米10年金利(^TNX)・原油(CL=F)を
  取得し、PCAの主成分スコアとの相関を計算してPCA解説文に組み込んでいる。
  実データで検証したところ、日本のPC1（説明力30%）はこの3変数いずれとも
  相関が弱く（|相関|<0.2）、「PC1=マクロ主導」と単純には言えないことが
  分かった。一方、米国のPC2は原油(+0.39)・米10年金利(+0.52)とそこそこ強い
  相関があり、金利敏感株のローテーションを捉えている可能性が高い。
  相関|0.4|以上の場合のみ「マクロ要因との連動が明確」と記載し、弱い場合は
  安易に特定のマクロ変数のせいにしない設計にしている。
- **8ファクターとの相互参照**: `decompose.py`のValue/Growth/Momentum等8
  ファクターと、PCAが見つけた主成分は別々に計算・表示している（層1の
  ファクターリターン表 と 層3のPCAセクション）。両者を定量的に突き合わせる
  相関チェックまではまだ実装していない（次の拡張候補）。
- **market_regime.py（既存の類似局面分析ツール）**: 独立したツールのまま
  据え置き。日次レポートへの統合は未着手（次の拡張候補）。
...
daily-review/
  src/
    ...
    macro.py                      マクロ変数（USDJPY・米10年金利・原油）とPCAスコアの相関
